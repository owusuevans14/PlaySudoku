let setLimits;
let boardSize = 4;
let level = 2; // 0 = Easy, 1 = Medium, 2 = 
let board, boxSize, isPuzzleValidate, solvedPuzzle, digger, questionPuzzle;
let view, sideMenuButton,solverPanel,sideMenuDiv;
let solver, startSolverButton,solverSpeedometerButton, solverStopButton ,watchSolverButton;
...
function transposePuzzle(board, boardSize, which) {
...
function copyPuzzle(board) {
    let copyOfPuzzle = Array.from(Array(board.length), () => new Array(board.length).fill(0))
    for (let i = 0; i < board.length; i++) {
        for (let j = 0; j < board.length; j++) {
